...
def version_num(params):
    """
    统计指定事件类型在各个版本中的数量，并将结果写入到指定文件中。

    Args:
        params (dict): 包含以下字段的字典：
            - sheet (list): 包含文件对象或类似文件对象，用于写入统计结果的列表。
            - data (pd.DataFrame): 包含版本和事件类型等信息的数据集。
            - exc (list): 包含需要统计的事件类型名称的列表。

    Returns:
        dict: 包含版本列表和每个事件类型对应的版本数量信息的字典。
...
def public_params(file):
    ...
    Returns:
        dict: 包含以下键值的字典：
            - exc (list): 包含异常事件类型及其描述的列表。
            - data (pandas.DataFrame): 读取Excel文件后得到的DataFrame，包含路测数据。
            - km (pandas.DataFrame): 读取Excel文件中“车辆里程信息”sheet后得到的DataFrame，包含自动驾驶里程信息。
            - km2 (int): 合计的自动驾驶里程（单位：km）。
            - master (pandas.DataFrame): 筛选版本为"8.3.4"的数据后得到的DataFrame。
            - rb (pandas.DataFrame): 筛选版本为"8.4.40"、"8.4.37"或"8.4.39"的数据后得到的DataFrame。
            - kk (object): 调用mb_km函数处理km后得到的返回值（类型取决于mb_km函数的定义）。
            - sheet (list): 包含三个工作表的列表，分别为version_num_sheet、sum_sheet和rb_sheet。
            - wb (openpyxl.workbook.Workbook): 创建的Workbook对象。
